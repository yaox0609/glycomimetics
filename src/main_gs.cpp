...
std::pair<double, std::vector<double> > GridSearchingGS(AtomVector& receptor_atoms, AtomVector& ligand_atoms, AtomVector& moiety_atoms, std::vector<AtomVector>& all_torsions, int rotation_interval, int num_threads, std::vector<double>& crystal_torsion_values, double chpi_weight);

bool InternalClashesExistGS(AtomVector& atom_set_1, AtomVector& atom_set_2, int coord_index);

void* GridSearchingByMultipleThreadsStartRoutineGS(void* arg_ptr);

bool GridSearchingByMultipleThreadsGS(int coord_index, std::vector<AtomVector>& all_torsions, std::vector<int>& thread_start_torsion_values, int interval, int num_rotamers_to_sample, double* highest_affinity_ptr, pthread_mutex_t* mutex_ptr, AtomVector& receptor_atoms, AtomVector& ligand_atoms, AtomVector& moiety_atoms, std::vector<double>* highest_affinity_torsions_ptr, std::vector<double>& crystal_torsion_values, double chpi_weight);
...
int main(int argc, char* argv[])
{
    ...
    std::ifstream torsion_file ("/home/yao/GLYCAM_Dev_Env/V_2/Web_Programs/gems/glycomimetics/good_chpi_gs/torsion_tokens_sugar.txt");
    ...
    std::cout << "Main interval: " << interval << std::endl;
    ...
    std::pair<double, std::vector<double> > top_affinity_torsion_values = (requested_residue_index_str == "-11") ? GridSearchingGS(receptor_atoms, ligand_atoms, ligand_atoms, torsions, interval, num_threads, crystal_torsion_values, chpi_weight) : GridSearchingGS(ligand_atoms, receptor_atoms, requested_residue_atoms, torsions, interval, num_threads, crystal_torsion_values, chpi_weight) ;
...
bool InternalClashesExistGS(AtomVector& atom_set_1, AtomVector& atom_set_2, int coord_index){ //This GS version does not exclude hydrogens
	...
	if (current_atom->GetElementSymbol() != "H"){
		    ...
		    if (another_atom->GetElementSymbol() != "H"){
				        ...
				        //std::cout << current_atom->GetResidue()->GetName() << "-" << current_atom->GetName() << " has a bond by distance neighbor " << another_atom->GetResidue()->GetName() << "-" << another_atom->GetName() << std::endl;
	    ...
	    for (unsigned int i = 0; i < neighbors.size(); i++){
		    ...
		    //std::cout << "Acutal nonH neighbor " << neighbors[i]->GetName() << std::endl;
		...
		//std::cout << "Clash with atom " << current_atom->GetName() << " bond by distance count " << bond_by_distance_count << "Non H neighbor count " << non_h_neighbors_count << std::endl;
	        return true;
            }
        }
    }
...
std::pair<double, std::vector<double> > GridSearchingGS(AtomVector& receptor_atoms, AtomVector& ligand_atoms, AtomVector& moiety_atoms, std::vector<AtomVector>& all_torsions, int interval, int num_threads, std::vector<double> & crystal_torsion_values, double chpi_weight){
	...
	int success_status = pthread_create(&tid[i], NULL, &GridSearchingByMultipleThreadsStartRoutineGS, &argstruct[i]);
    }
...
bool GridSearchingByMultipleThreadsGS(int coord_index, std::vector<AtomVector>& all_torsions, std::vector<int>& thread_start_torsion_values, int interval, int num_rotamers_to_sample, double* highest_affinity_ptr, pthread_mutex_t* mutex_ptr, AtomVector& receptor_atoms, AtomVector& ligand_atoms, AtomVector& moiety_atoms, std::vector<double>* highest_affinity_torsions_ptr, std::vector<double>& crystal_torsion_values, double chpi_weight){
    ...
    /*AtomVector ligand_atoms_within_8_angstroms;
    for (unsigned int i = 0; i < moiety_atoms.size(); i++){
        MolecularModeling::Atom* this_moiety_atom = moiety_atoms[i];
	GeometryTopology::Coordinate* moiety_atom_coord = this_moiety_atom->GetCoordinates()[coord_index];
	
	for (unsigned int j = 0; j < ligand_atoms.size(); j++){
	    MolecularModeling::Atom* this_ligand_atom = ligand_atoms[j];
	    GeometryTopology::Coordinate* ligand_atom_coord = this_ligand_atom->GetCoordinates()[coord_index];

	    double dist = moiety_atom_coord->Distance(ligand_atom_coord);
	    if (dist < 8){
	        if (std::find(ligand_atoms.begin(), ligand_atoms.end(), this_ligand_atom) == ligand_atoms_within_8_angstroms.end()){
		    ligand_atoms_within_8_angstroms.push_back(this_ligand_atom);
		}
	    }
	...
	unsigned int i = (rotamer_sampled == 0) ? rotation_values[all_torsions.size() -1] : start_value;
        for (; i <= end_value; i += interval){
            ...
            if (!InternalClashesExistGS(moiety_atoms, ligand_atoms, coord_index)){
	    ...
	    for (unsigned int i = 0; i < all_torsions.size(); i++){
                std::cout << GetDihedral(all_torsions[i][0], all_torsions[i][1], all_torsions[i][2], all_torsions[i][3], coord_index) - crystal_torsion_values[i] << "-";
	    }
	    std::cout << std::endl;*/
